    ...
    <div class="success-card">
      @if($order)
        ...
        <h2 class="success-title">
          {{ trans('front/payment.success_title') ?? 'Thank You!' }}
        </h2>
        <p class="success-subtitle">{{ __('front/payment.success_subtitle') ?? 'Your order has been received successfully.' }}</p>
        ...
        <div class="order-details-grid">
          ...
          <div class="order-detail-item">
            ...
            <h5><span class="badge bg-success bg-opacity-25 text-success rounded-pill px-3">{{ $order->status_format ?? $order->status }}</span></h5>
          </div>
        </div>
        ...
        <div class="success-actions">
          @if(current_customer())
            <a href="{{ account_route('orders.number_show', ['number'=>$order->number]) }}" class="btn-success-primary">
              {{ trans('front/payment.view_order') ?? 'View Order' }}
            </a>
          @else
          ...
          <a href="{{ front_route('home.index') }}" class="btn-success-secondary">
            {{ trans('front/payment.continue_shopping') ?? 'Continue Shopping' }}
          </a>
        </div>
      @else
