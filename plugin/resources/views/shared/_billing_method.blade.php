...
@push('footer')
    <script>
        $(function () {
            ...
            axios.get("{{config('Travalorics.api_url').'/api/checkout/billing_methods'}}").then(function (res) {
            ...
            $('#quickBuy').on('click',function () {
                ...
                axios.post('/{{ panel_name() }}/marketplaces/quick_checkout', {
                    ...
                    window.open('{{config('Travalorics.api_url')}}' + '/orders/' + res.data.number + '/pay', '_blank')
                })
            })
        })
    </script>
