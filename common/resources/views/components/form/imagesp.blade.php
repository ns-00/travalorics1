<x-panel::form.row :title="$title">
    <div class="multi-images-pure-upload-wrapper">
      ...
      <i class="bi bi-info-circle"></i> {{ __('panel/common.up_image_text') }}
            @if ($max)
                , most uploaded(<span class="imgs-count">{{ count($values) }}</span>/{{ $max }})open
            @endif
    </span>
        {{ $slot }}
    </div>
                ...
                axios.post('{{ front_root_route('upload.images') }}', formData)
                    .then(function (response) {
                        if (response.success) {
                            ...
                            layer.msg(response.message || 'Upload failed', {icon: 2});
                        }
                    })
                    .catch(function (error) {
                        layer.msg(error.response?.message || 'Upload failed', {icon: 2});
                    })
                    .finally(function () {
