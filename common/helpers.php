...
use Travalorics\Common\Libraries\ApiHook;
use Travalorics\Common\Libraries\Currency;
use Travalorics\Common\Libraries\ViewHook;
use Travalorics\Common\Libraries\Weight;
use Travalorics\Common\Repositories\CurrencyRepo;
use Travalorics\Common\Repositories\LocaleRepo;
use Travalorics\Common\Repositories\SettingRepo;
use Travalorics\Common\Services\ImageService;
use Travalorics\Common\Support\Registry;
...
if (! function_exists('load_settings')) {
    ...
    function load_settings(): void
    {
        ...
        if (config('travalorics.system')) {
            return;
        }

        $result = SettingRepo::getInstance()->groupedSettings();
        config(['travalorics' => array_merge((array) config('travalorics', []), $result)]);
    }
}
...
if (! function_exists('setting')) {
    ...
    {
        return config("travalorics.{$key}", $default);
    }
}
...
if (! function_exists('travalorics_path')) {
    /**
     * Get innopack path
     *
     * @param  string  $path
     * @return string
     */
    function travalorics_path(string $path): string
    {
        return base_path("travalorics/{$path}");
    }
}
...
if (! function_exists('token_customer_id')) {
    ...
    {
        $user = request()->user('sanctum') ?? request()->user('customer') ?? auth('customer')->user() ?? request()->user();
        return $user->id ?? 0;
    }
}
...
if (! function_exists('token_customer')) {
    ...
    {
        return request()->user('sanctum') ?? request()->user('customer') ?? auth('customer')->user() ?? request()->user();
    }
}
...
if (! function_exists('current_locale')) {
    ...
    {
        return locales()->where('code', front_locale_code())->first();
    }
}
...
if (! function_exists('travalorics_view')) {
    /**
     * @param  null  $view
     * @param  array  $data
     ...
     * @return mixed
     */
    function travalorics_view($view = null, array $data = [], array $mergeData = []): mixed
    {
...
if (! function_exists('Travalorics_version')) {
    /**
     * Generate an asset path for the application.
     *
     * @return string
     */
    function Travalorics_version(): string
    {
        $default = ucfirst(config('Travalorics.edition')).' v'.config('Travalorics.version').'('.config('Travalorics.build').')';

        return fire_hook_filter('Travalorics.version.display', $default);
    }
}

if (! function_exists('Travalorics_brand_link')) {
    /**
     * Get Travalorics brand link
     *
     * @return string
     */
    function Travalorics_brand_link(): string
    {
        if (is_admin()) {
            $default = '<a href="https://www.Travalorics.com" class="ms-2" target="_blank">Travalorics</a>';
        } else {
            $default = 'Powered By <a href="https://www.Travalorics.com" class="ms-2" target="_blank">Travalorics</a>';
        }

        return fire_hook_filter('Travalorics.brand.link.display', $default);
    }
}
...
if (! function_exists('seller_enabled')) {
    ...
    {
        return class_exists('\Travalorics\Seller\SellerServiceProvider') && env('SELLER_ENABLED', true);
    }
}
