...
use Travalorics\Common\Repositories\CartItemRepo;
use Travalorics\Common\Resources\CartListItem;
...
class CartService
{
    ...
    public function addCart($data): array
    {
        ...
        if (! empty($data['options'])) {
            $productSku = \Travalorics\Common\Models\Product\Sku::find($data['sku_id']);
            if ($productSku && $productSku->product) {
                $optionService = \Travalorics\Common\Services\ProductOptionService::getInstance($productSku->product);
    ...
    {
        // 先删除已有的选项值
        \Travalorics\Common\Models\Cart\OptionValue::where('cart_item_id', $cartItem->id)->delete();
        ...
        foreach ($options as $optionId => $optionValueIds) {
            ...
            foreach ($optionValueIds as $optionValueId) {
                // 获取选项和选项值信息
                $option      = \Travalorics\Common\Models\Option::find($optionId);
                $optionValue = \Travalorics\Common\Models\OptionValue::find($optionValueId);
                ...
                // 获取产品选项值配置中的价格调整
                $productOptionValue = \Travalorics\Common\Models\Product\OptionValue::where([
                ...
                // 创建购物车选项值记录
                \Travalorics\Common\Models\Cart\OptionValue::create([
