...
use Travalorics\Common\Repositories\Customer\GroupRepo;
...
class CustomerRepo extends BaseRepo
{
    ...
    public function builder(array $filters = []): Builder
    {
        ...
        if ($createdStart) {
            $start = str_contains($createdStart, ' ') ? $createdStart : $createdStart . ' 00:00:00';
            $builder->where('created_at', '>=', $start);
        }

        $createdEnd = $filters['created_at_end'] ?? '';
        if ($createdEnd) {
            $end = str_contains($createdEnd, ' ') ? $createdEnd : $createdEnd . ' 23:59:59';
