...
use Travalorics\Common\Repositories\Product\BundleRepo;
use Travalorics\Common\Repositories\Product\OptionValueRepo;
use Travalorics\Common\Repositories\Product\RelationRepo;
use Travalorics\Common\Repositories\Product\VariantRepo;
...
class ProductRepo extends BaseRepo
{
    ...
    private function createOrUpdate(Product $product, $data): mixed
    {
        ...
        try {
            ...
            if (isset($data['price_type']) && $data['price_type'] === 'single' && ! empty($skus)) {
                $currentVars = is_array($product->variables) ? $product->variables : [];
                $newVars = [];
                if (isset($currentVars['review_criteria'])) {
                    $newVars['review_criteria'] = $currentVars['review_criteria'];
                }
                if (isset($currentVars['review_criteria_en'])) {
                    $newVars['review_criteria_en'] = $currentVars['review_criteria_en'];
                }
                $product->update(['variables' => $newVars]);
                $skus = [$skus[0]];
            }
    ...
    public function builder(array $filters = []): Builder
    {
        ...
        $queryBuilder = new \Travalorics\Common\Services\ProductQueryBuilder;
    ...
    public function getBestSellerProducts(int $limit = 8): mixed
    {
        return $this->withActive()->builder()
            ...
            ->orderByRaw('(sales + order_items_count) DESC')
            ->limit($limit)
            ->get();
    }
