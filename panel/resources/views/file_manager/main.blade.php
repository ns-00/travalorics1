@push('header')
  ...
  <script>
    ...
    (function(window) {
      ...
      let fileManagerLoadingCount = 0;
      let fileManagerLoadingIndex = null;

      const showFileManagerLoading = () => {
        if (fileManagerLoadingIndex === null && window.layer) {
          fileManagerLoadingIndex = layer.load(2, {
            shade: [0.3, '#fff']
          });
        }
      };

      const hideFileManagerLoading = () => {
        fileManagerLoadingCount = Math.max(0, fileManagerLoadingCount - 1);
        if (fileManagerLoadingCount === 0 && fileManagerLoadingIndex !== null && window.layer) {
          layer.close(fileManagerLoadingIndex);
          fileManagerLoadingIndex = null;
        }
      };

      const resetFileManagerLoading = () => {
        fileManagerLoadingCount = 0;
        if (fileManagerLoadingIndex !== null && window.layer) {
          layer.close(fileManagerLoadingIndex);
          fileManagerLoadingIndex = null;
        }
      ...
      window.addEventListener('pageshow', resetFileManagerLoading);
      window.addEventListener('pagehide', resetFileManagerLoading);
    })(window);
  </script>

  <style>
    /* ═══ File Manager – Premium Design ════════════════════ */
    :root {
      --fm-primary: #6F4E37;
      --fm-primary-light: #8B6347;
      --fm-primary-rgb: 111,78,55;
      --fm-bg: #F7F8FA;
      --fm-card: #FFFFFF;
      --fm-border: #E8ECF0;
      --fm-text: #1A202C;
      --fm-muted: #718096;
      --fm-radius: 10px;
      --fm-shadow: 0 2px 12px rgba(0,0,0,0.08);
    }
    ...
    .folder-tree::-webkit-scrollbar-thumb { background: rgba(var(--fm-primary-rgb),0.2); border-radius: 4px; }
    ...
    .file-list::-webkit-scrollbar-thumb { background: rgba(var(--fm-primary-rgb),0.15); border-radius: 4px; }
    ...
    .file-card:hover {
      ...
      box-shadow: 0 4px 16px rgba(var(--fm-primary-rgb),0.12);
    ...
    .file-card:hover .file-thumb { background: rgba(var(--fm-primary-rgb),0.04); }
    ...
    .el-tree-node.is-current>.el-tree-node__content {
      background-color: rgba(111,78,55,0.08) !important;
    ...
    @media (max-width: 768px) {
      ...
      .file-toolbar .el-button {
        padding: 5px 8px !important;
        font-size: 12px !important;
      }
      .file-toolbar .el-button-group { flex-wrap: wrap; }

      .folder-tree { min-width: 120px; font-size: 13px; }

      .file-list { padding: 10px !important; height: 75vh !important; }

      .file-card { margin-bottom: 8px; }
      .file-thumb { height: 100px !important; }
      .file-name { font-size: 12px !important; }
      .file-type { font-size: 10px !important; }
      .file-info { padding: 7px 9px !important; }

      .el-col-6 { width: 50% !important; }

      /* إخفاء عمود معلومات المساحة */
      .fm-space-info, .el-col-6:last-child:not(:first-child) .el-card { display: none !important; }

      .pagination-container { padding: 10px !important; }
      .el-pagination { font-size: 12px !important; }
      .el-pagination .el-pager li { min-width: 28px !important; height: 28px !important; line-height: 28px !important; }
    }

    @media (max-width: 480px) {
      .file-thumb { height: 80px !important; }
      .el-col-6 { width: 50% !important; }
      .el-button--small { padding: 5px 7px !important; font-size: 11px !important; }
    }
    ...
    .file-card.selected {
      ...
      background: rgba(var(--fm-primary-rgb),0.04) !important;
      box-shadow: 0 0 0 3px rgba(var(--fm-primary-rgb),0.15) !important;
    }
    .fm_old_selected {
      border-color: var(--fm-primary);
    ...
    /* Final responsive guardrails for the live grid. The Vue bindings above
       remain unchanged; these rules only control presentation and flow. */
    .file-toolbar-row,
    ...
    .file-toolbar-row > .el-col,
    .file-grid-row > .el-col {
      min-width: 0;
    }
    ...
    @media (max-width: 767px) {
      ...
      .file-toolbar-row > .el-col:last-child {
        margin-bottom: 0;
      }

      .file-toolbar .el-button-group {
        display: flex;
        flex-wrap: wrap;
        gap: 6px;
        margin: 0;
      ...
      .file-toolbar .el-button-group .el-button {
        margin-left: 0 !important;
        border-radius: 6px !important;
      }

      .file-toolbar .el-select {
    ...
    .el-tree-node.is-drop-inner>.el-tree-node__content {
      background-color: rgba(111,78,55,0.08) !important;
      border: 2px dashed var(--fm-primary) !important;
      animation: dropTarget 1s ease infinite;
    }
  ...
  <div id="app">
    <div class="file-manager">
      <div class="file-toolbar">
        <el-row class="file-toolbar-row" type="flex" justify="space-between" align="middle">
          <el-col :span="12">
            <el-button-group>
              <el-button type="primary" size="small" @click="uploadFile">
                <i class="el-icon-upload2"></i> رفع ملف
              </el-button>
              ...
              <el-button size="small" :type="isMultiSelectMode ? 'primary' : 'default'" @click="toggleMultiSelectMode">
                <i class="el-icon-check"></i> وضع التحديد المتعدد
              </el-button>
              <el-button v-if="isMultiSelectMode" size="small" @click="selectAll">
                <i class="el-icon-finished"></i> تحديد الكل
              </el-button>

              <el-button type="primary" size="small" @click="handleConfirm">اختيار وإرسال</el-button>
          ...
          <el-col :span="12" style="text-align: right">
            <el-button-group>
              <el-button size="small" :disabled="selectedFiles.length !== 1" @click="renameSelectedFile">
                <i class="el-icon-edit"></i> إعادة تسمية
              </el-button>
              ...
              <el-button size="small" :disabled="!selectedFiles.length" @click="moveFiles">
                <i class="el-icon-folder"></i> نقل إلى
              </el-button>
            ...
            <el-select v-model="sortField" size="small" style="width:110px;margin-left:10px;" @change="onSortChange">
              <el-option label="حسب الوقت" value="created"></el-option>
              <el-option label="حسب الاسم" value="name"></el-option>
            </el-select>
            <el-select v-model="sortOrder" size="small" style="width:90px;margin-left:5px;" @change="onSortChange">
              <el-option label="تنازلي" value="desc"></el-option>
              <el-option label="تصاعدي" value="asc"></el-option>
            </el-select>
          </el-col>
        </el-row>
      </div>
      <el-row class="file-manager-columns" :gutter="0">
        <!-- 左侧文件夹树 -->
              ...
              <div class="el-tree-node__wrapper" slot-scope="{ node, data }"
                ...
                <span class="custom-tree-node">
                  <i :class="[
                    ...
                    style="margin-right: 4px; color: var(--fm-primary);"></i>
        ...
        <el-col :xs="16" :sm="18" :md="19" :lg="20"
          ...
          <div class="file-list">
            <div class="file-list-content">
              <div v-loading="loading" element-loading-text="جاري التحميل...">
                <el-row class="file-grid-row" :gutter="20">
                  <el-col :xs="12" :sm="8" :md="6" :lg="4"
                    ...
                    <div :class="['file-card', selectedFiles.includes(file.id || file.path) ? 'selected' : '']"
                      ...
                      <div class="file-info">
                        ...
                        <p class="file-type">@{{ file.is_dir ? 'مجلد' : file.mime }}</p>
                      </div>
                    </div>
                  </el-col>
                </el-row>
                ...
                <!-- 添加空状态 -->
                <el-empty v-else description="لا توجد ملفات" :image-size="120"></el-empty>
              </div>
            </div>
          </div>
        </el-col>
    ...
    <!-- 新建文件夹对话框 -->
    <el-dialog title="إنشاء مجلد جديد" :visible.sync="folderDialog.visible" width="400px">
    ...
    <!-- 上传文件对话框 -->
    <el-dialog title="تحميل الملفات" :visible.sync="uploadDialog.visible" width="500px" @open="onUploadDialogOpen">
      <el-upload class="file-uploader" drag multiple :action="uploadUrl" :headers="uploadHeaders"
        ...
        <div class="el-upload__text">اسحب الملف إلى هنا، أو<em>انقر للتحميل</em></div>
        <div class="el-upload__tip" slot="tip">يدعم jpg、jpeg、png、gif、webp、mp4 ملف التنسيق</div>
        <div class="el-upload__tip" slot="tip">الحد الأقصى لحجم الملف الذي تم تحميله إلى الخادم: {{ $uploadMaxFileSize ?? 'مجهول' }}</div>
        <div class="el-upload__tip" slot="tip">الحد الأقصى لحجم بيانات POST: {{ $postMaxSize ?? 'مجهول' }}</div>
      </el-upload>
    </el-dialog>

    <!-- 修改重命名对话框 -->
    <el-dialog title="إعادة تسمية" :visible.sync="renameDialog.visible" custom-class="rename-dialog" width="500px">
      ...
      <span slot="footer">
        <el-button @click="renameDialog.visible = false">يلغي</el-button>
        <el-button type="primary" @click="submitRename">بالتأكيد</el-button>
      </span>
    </el-dialog>

    <!-- 移动文件对话框 -->
    <el-dialog title="الانتقال إلى" :visible.sync="moveDialog.visible" width="400px">
      <el-tree :data="folders" :props="defaultProps" @node-click="handleMoveTargetSelect"
        :highlight-current="true" node-key="id">
        <span class="custom-tree-node" slot-scope="{ node, data }">
          <i class="el-icon-folder" style="margin-right: 4px; color: var(--fm-primary);"></i>
          <span>@{{ node.label }}</span>
        </span>
      </el-tree>
     <span slot="footer">
  <el-button @click="moveDialog.visible = false">إلغاء</el-button>
...
<div class="file-card-context-menu" v-show="contextMenu.visible" :style="contextMenu.style">
  <ul>
    ...
    <li @click="moveFile"><i class="el-icon-folder"></i> نقل إلى</li>
    <li @click="copyFile"><i class="el-icon-document-copy"></i> نسخ إلى</li>
  </ul>
</div>

<!-- 复制文件对话框 -->
<el-dialog title="نسخ إلى" :visible.sync="copyDialog.visible" width="400px">
  ...
  <span slot="footer">
    <el-button @click="copyDialog.visible = false">إلغاء</el-button>
...
<div v-if="folderContextMenu.visible" class="file-card-context-menu"
  ...
  <ul>
    ...
    <li @click="moveFolder">
      <i class="el-icon-position"></i> نقل إلى
    </li>
...
<!-- 文件夹重命名对话框 -->
<el-dialog title="إعادة تسمية المجلد" :visible.sync="folderRenameDialog.visible" width="400px">
  <el-form :model="folderRenameDialog.form" label-width="80px">
    <el-form-item label="اسم المجلد">
      <el-input v-model="folderRenameDialog.form.newName" placeholder="الرجاء إدخال الاسم الجديد"></el-input>
    </el-form-item>
  </el-form>
  <span slot="footer">
    <el-button @click="folderRenameDialog.visible = false">إلغاء</el-button>
    <el-button type="primary" @click="submitFolderRename">تأكيد</el-button>
  </span>
</el-dialog>

<!-- 文件夹移动对话框 -->
<el-dialog title="نقل المجلد" :visible.sync="folderMoveDialog.visible" width="400px">
  ...
  <span slot="footer">
    <el-button @click="folderMoveDialog.visible = false">إلغاء</el-button>
              ...
              <div v-if="storageConfig.driver === 'oss'">
                ...
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control" id="ossEndpoint" v-model="storageConfig.endpoint"
                    placeholder="例如: https://Travalorics.oss-cn-hangzhou.aliyuncs.com">
                </div>
      ...
      methods: {
        ...
        loadFolders() {
          http.get('file_manager/directories').then(res => {
            ...
            this.folders = [{
              id: '/',
              name: '<span class="fm-space-text">图片空间</span>',
              path: '/',
              isRoot: true,
