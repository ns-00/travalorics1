...
<form class="needs-validation" novalidate action="{{ panel_route('settings.update') }}" method="POST" id="app-form">
  ...
  <div class="row">
    <div class="col-md-3">
      <div class="premium-dashboard-card p-0" id="setting-menu" style="overflow: hidden;">
        <div class="card-header bg-transparent fw-bold border-bottom pb-3 pt-4 px-4">{{ __('panel/menu.settings') }}</div>
    ...
    <div class="col-md-9">
      <div class="premium-dashboard-card h-min-600 p-0">
        <div class="card-header setting-header bg-transparent fw-bold border-bottom pb-3 pt-4 px-4">{{ __('panel/setting.basic') }}</div>
        <div class="card-body p-4">
...
<script>
  ...
  function getCountries() {
    axios.get('{{ front_route('countries.index') }}').then(function(res) {
      ...
      countrySelect.empty();
      countrySelect.append('<option value="">{{ __('panel/common.please_choose') }} {{ __('panel/common.country') }}</option>');
  ...
  function getZones(countryId) {
    axios.get('{{ front_route('countries.index') }}/' + countryId).then(function(res) {
      ...
      zoneSelect.append('<option value="">{{ __('panel/common.please_choose') }} {{ __('panel/common.state') }}</option>');
  ...
  function addSlide(btn) {
    ...
    var tr = `
      <tr>
        ...
        <td class="text-end">
          <button type="button" class="btn btn-danger" onclick="this.closest('tr').remove()">{{ __('panel/common.delete') }}</button>
        </td>
      </tr>
    `;
    tbody.append(tr);
