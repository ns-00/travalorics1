...
@section('content')
  <div class="premium-dashboard-card h-min-600" id="app">
    <div class="card-body p-0">
      ...
      <div class="mb-4 p-3 bg-light bg-opacity-50 rounded-4 border" id="products-toolbar">
          ...
          <table class="table align-middle">
            ...
            @foreach ($products as $product)
              <tr>
                ...
                <td>
                  <div class="d-flex align-items-center justify-content-center border border-light rounded-3 shadow-sm bg-white overflow-hidden" style="width: 55px; height: 55px;">
                    <a href="{{ $product->url }}" target="_blank">
                      <img src="{{ image_resize($product->image_url, 60, 60) }}" class="img-fluid w-100 h-100" style="object-fit: cover;"
                           alt="{{ $product->fallbackName() }}">
                    </a>
                  </div>
                </td>
                <td>
                  <a href="{{ $product->url }}" class="text-decoration-none fw-bold text-dark hover-primary transition-all" target="_blank" data-bs-toggle="tooltip"
                     title="{{ $product->fallbackName() }}">
                    {{ sub_string($product->fallbackName(),28) }}
                  </a>
                  @if($product->isMultiple())
                    &nbsp;<span class="badge bg-success bg-opacity-10 text-success px-2 rounded-pill border border-success border-opacity-25">M</span>
                  @endif
                </td>
                <td class="fw-bold text-success">{{ currency_format($product->masterSku->price ?? 0) }}</td>
