    ...
    @foreach($product->bundles ?? [] as $i => $bundle)
      <div class="bundle-item d-flex align-items-center mb-2">
        <div class="input-group flex-grow-1">
          <input type="text" class="form-control sku-autocomplete" name="bundles[{{ $i }}][sku_code_text]" value="{{ old("bundles.{$i}.sku_code_text", $bundle->sku->full_name ?? '') }}" placeholder="{{ __('panel/product.bundle_sku_placeholder') }}" autocomplete="off" required>
          <input type="hidden" name="bundles[{{ $i }}][sku_id]" value="{{ old("bundles.{$i}.sku_id", $bundle->sku_id) }}">
        </div>
        <input type="number" class="form-control mx-2" name="bundles[{{ $i }}][quantity]" value="{{ old("bundles.{$i}.quantity", $bundle->quantity) }}" min="1" required style="width:100px;">
...
$(function () {
  ...
  $(document).on('focus', '.sku-autocomplete', function() {
    if (!$(this).data('ui-autocomplete')) initSkuAutocomplete();
  });
