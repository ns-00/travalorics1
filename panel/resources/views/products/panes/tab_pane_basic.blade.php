<div class="tab-pane fade show active mt-3" id="basic-tab-pane" role="tabpanel" aria-labelledby="basic-tab" tabindex="0">
  <div class="mb-3 col-12 col-md-5">
    ...
    <select class="form-select" name="type" id="product-type" {{ $product->id ? 'disabled' : '' }} required>
      @php
        $productTypes = \Travalorics\Common\Repositories\ProductRepo::getProductTypes();
        $currentType = old('type', $product->type ?? 'normal');
      @endphp
      @foreach($productTypes as $typeValue => $typeLabel)
        <option value="{{ $typeValue }}" {{ $currentType == $typeValue ? 'selected' : '' }}>{{ $typeLabel }}</option>
      @endforeach
    </select>
    @if($product->id)
      <input type="hidden" name="type" value="{{ old('type', $product->type ?? 'normal') }}">
    @endif
    <div class="mt-2 text-muted small"><i class="bi bi-info-circle me-1"></i>{{ __('panel/product.type_hint') }}</div>
  </div>

  <div class="mb-3 col-12 col-md-5">
    <div class="mb-1 fs-6">{{ __('panel/product.name') }}</div>
    @if(has_translator())
      <div class="d-flex align-items-center my-3 py-2 px-3 text-primary-emphasis bg-primary-subtle border border-primary-subtle rounded-3" style="white-space: nowrap;">
        ...
        <button type="button" class="mx-2 btn btn-primary btn-custom-small btn-sm" id="translate-button">{{ __('panel/product.translate') }}</button>
      </div>
    @endif
      ...
      <div class="input-group mb-2">
        <div class="input-group-text">
          <div class="d-flex align-items-center wh-20">
            <img src="{{ image_origin($locale->image) }}" class="img-fluid {{ default_locale_class($locale->code) }}" alt="{{ $localeName }}">
          </div>
        </div>
    ...
    @endforeach
    <div class="mt-1 text-muted small"><i class="bi bi-info-circle me-1"></i>{{ __('panel/product.name_required_hint') }}</div>
  </div>

  <x-common-form-images title="{{ __('panel/common.image') }}" name="images" :values="old('images', $product->images ?? [])"/>
  <x-common-form-image title="{{ __('panel/product.hover_image') }}" name="hover_image" :value="old('hover_image', $product->hover_image ?? '')" :description="__('panel/product.hover_image_help')"/>
    ...
    <div class="col-12">
      <div class="mb-3" id="spec-type-group">
        <div class="form-check form-check-inline">
          <input class="form-check-input" type="radio" name="price_type" id="price_type_single" value="single" {{ (!$product->isMultiple()) ? 'checked' : '' }}>
          <label class="form-check-label" for="price_type_single">{{ __('panel/product.price_type_single') }}</label>
        </div>
        <div class="form-check form-check-inline">
          <input class="form-check-input" type="radio" name="price_type" id="price_type_multiple" value="multiple" {{ ($product->isMultiple()) ? 'checked' : '' }}>
          <label class="form-check-label" for="price_type_multiple">{{ __('panel/product.price_type_multiple') }}</label>
        </div>
      </div>
      ...
      <div id="single_price_box" class="{{ $product->isMultiple() ? 'd-none' : '' }}">
        <div class="row">
          <div class="col-12 col-md-3">
            <x-common-form-input :title="__('panel/product.price')" name="skus[0][price]" value="{{ old('skus.0.price', $product->masterSku->price ?? '') }}" required/>
          </div>
          <div class="col-12 col-md-3">
            <x-common-form-input :title="__('panel/product.quantity')" name="skus[0][quantity]" value="{{ old('skus.0.quantity', $product->masterSku->quantity ?? '') }}" required/>
          </div>
          <div class="col-12 col-md-3">
            <x-common-form-input :title="__('panel/product.sku_code')" name="skus[0][code]" value="{{ old('skus.0.code', $product->masterSku->code ?? '') }}" required/>
          </div>
          <div class="col-12 col-md-3">
      ...
      <div id="specifications_box" class="{{ !$product->isMultiple() ? 'd-none' : '' }}">
        <div class="alert alert-info mb-3" id="multi_spec_notice">
          <i class="bi bi-info-circle me-2"></i>{{ __('panel/product.multiple_spec_notice') }}
        </div>
