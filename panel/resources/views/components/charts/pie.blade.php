<div class="card mb-3">
  <div class="card-header d-flex justify-content-between align-items-center" style="height: 4rem;">
    ...
    <div class="btn-group">
      <button class="btn btn-sm btn-primary active" onclick="switchSource{{ $id }}('pc_web')" id="source_btn_{{ $id }}_pc_web">صفحة ويب للكمبيوتر</button>
      <button class="btn btn-sm btn-success" onclick="switchSource{{ $id }}('mobile_web')" id="source_btn_{{ $id }}_mobile_web">صفحة ويب للهاتف</button>
      <button class="btn btn-sm btn-info" onclick="switchSource{{ $id }}('miniapp')" id="source_btn_{{ $id }}_miniapp">التطبيق المصغر</button>
      <button class="btn btn-sm btn-warning" onclick="switchSource{{ $id }}('wechat_official')" id="source_btn_{{ $id }}_wechat_official">حساب WeChat الرسمي</button>
      <button class="btn btn-sm btn-danger" onclick="switchSource{{ $id }}('app')" id="source_btn_{{ $id }}_app">تطبيق APP</button>
    </div>
    @endif
  </div>
  <div class="card-body">
...
<script>
  ...
  // تهيئة بيانات المصدر الأول عند التحميل
  ...
  function removeData{{ $id }}() {
    ...
    if (chart) {
      ...
      if (data.labels.length > 1) {  // الاحتفاظ بنقطة بيانات واحدة على الأقل
        data.labels.pop();
