...
const UI = {
  ...
  initSidebar: () => {
    ...
    $(document).on("keydown", ".mb-menu", function (event) {
    ...
    $sidebar.on("click", function (event) {
      if (!$(event.target).closest(".sidebar-body").length) closeSidebar();
    });
    $(document).on("click", ".sidebar-body a", function () {
...
const FileManager = {
  init: function(callback, options = {}) {
    ...
    layer.open({
      ...
      shade: 0.8,
      area: ["90%", "90%"],
      content: `${typeof urls !== 'undefined' ? urls.base_url.replace(/\/$/, '') : '/panel'}/file_manager/iframe?type=${finalOptions.type}&multiple=${finalOptions.multiple ? "1" : "0"}`,
    });
  }
};
  ...
  setupEditor: (ed) => {
    ...
    ed.on("paste", (e) => {
      ...
      for (let i = 0; i < clipboardData.items.length; i++) {
        ...
        axios
          .post((typeof urls !== 'undefined' ? urls.api_base.replace(/\/$/, '') + '/file_manager/upload' : 'api/panel/file_manager/upload'), formData)
          .then((response) => {
