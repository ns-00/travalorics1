let formLoadingLayerIndex = null;

function closeFormLoading() {
  if (formLoadingLayerIndex !== null && window.layer) {
    window.layer.close(formLoadingLayerIndex);
    formLoadingLayerIndex = null;
  }
}

window.addEventListener('pageshow', closeFormLoading);
window.addEventListener('pagehide', closeFormLoading);
...
$(function () {
  ...
  $(document).on('submit', 'form', function(event) {
    if (!$(this).hasClass('no-load') && formLoadingLayerIndex === null) {
      formLoadingLayerIndex = layer.load(2, { shade: [0.2, '#fff'] });
    }
  });
