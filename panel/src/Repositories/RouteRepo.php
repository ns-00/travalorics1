...
class RouteRepo
{
    ...
    public function getPanelPermissions(): array
    {
        ...
        foreach ($panelRoutes as $module => $routes) {
            ...
            if ($isSystem) {
                ...
                $label  = is_array(trans("$module::route.title")) ? $module : (trans("$module::route.title") ?? $module);
            }
            $permissions[] = [
                'is_plugin'   => ! $isSystem,
                'module'      => $module,
    ...
    public function getPanelRoutes(): array
    {
        ...
        foreach ($routes as $route) {
            ...
            if (! Str::startsWith($route->getName(), '/travalorics-admin.')) {
                continue;
            }

            $routeName = substr($routeName, strlen('/travalorics-admin.'));
    ...
    {
        if (Str::startsWith($controller, 'Travalorics\\Panel')
            || Str::startsWith($controller, 'Travalorics\\Plugin')) {
            return '';
        }
