...
namespace Travalorics\Panel\Components\Layout;

use Illuminate\View\Component;
use Travalorics\Plugin\Repositories\PluginTypeRepo;
...
class Sidebar extends Component
{
    ...
    private function getMenus(): array
    {
        $menus = [
            ...
            // Shipping Extension (Level-3 Integration)
            [
                'title'    => __('panel/menu.shipping'),
                'icon'     => 'bi-truck',
                'prefixes' => ['shipping'],
                'children' => $this->getShippingSubRoutes(),
            ],
    ...
    public function getAnalyticSubRoutes(): array
    {
        $routes = [
            ...
            ['route' => 'reports.index', 'title' => 'التقارير'],
        ];
    ...
    public function getSettingSubRoutes(): array
    {
        $routes = [
            ...
            ['route' => 'custom_shipping.index', 'title' => 'الشحن المخصص'],
        ];
    ...
    {
        $routes = [
            ['route' => 'coupons.index', 'title' => __('panel/coupon.coupons')],
        ];
    ...
    /**
     * Get shipping sub routes (Enterprise Architecture).
     */
    public function getShippingSubRoutes(): array
    {
        $routes = [];

        if (\Illuminate\Support\Facades\Route::has('panel.shipping.dhl.index')) {
            $routes[] = [
                'route' => 'plugins.index', 
                'url' => route('panel.shipping.dhl.index'), 
                'title' => __('panel/menu.dhl_control_center'),
                'active' => request()->routeIs('panel.shipping.dhl.*')
            ];
        }

        return fire_hook_filter('panel.component.sidebar.shipping.routes', $routes);
    }
}
